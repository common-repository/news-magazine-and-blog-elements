...
if ( ! class_exists(' NewsGreen_Widget_Helper' ) ) {
	...
	* @since 1.0.0
	*/
	class eD_News_Widget_Helper extends WP_Widget {
			...
			switch ( $type ) {
				...
				case 'image':
					...
					<div class="widget-field-image">
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?></label>
						<input type="button" class="button button-primary wh-image-picker" value="<?php esc_attr_e( 'Upload', 'newsgreen' ); ?>" data-uploader_title="<?php esc_attr_e( 'Select Image', 'newsgreen' ); ?>" data-uploader_button_text="<?php esc_attr_e( 'Choose Image', 'newsgreen' ); ?>" />
						...
						<input type="button" class="button button-secondary btn-image-remove" value="<?php esc_attr_e( 'Remove', 'newsgreen' ); ?>" <?php echo ( empty( $value ) ) ? 'style="display:none;"' : ''; ?> />
				...
				case 'checkbox':
					?>
					<p>
						<input disabled="disabled" type="checkbox" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" class="<?php echo esc_attr( $class ); ?>" <?php checked( ! empty( $value ) ); ?> />
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?> <span class="pro_text widget-field-description"><a href="<?php echo ATA_PRO;?>" target="_blank">upgrade pro to unlock</a></span></label>
					</p>
				...
				case 'select':
					...
					?>
                    <?php if( $key == 'style' ):?>
                    <p class="widget-field-select ata_wrp_select">
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?></label>
					...
					<p class="widget-field-select ata_wrp_select">
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?></label>
						<select id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" class="<?php echo esc_attr( $class ); ?>" disabled="disabled">
							<?php if ( ! empty( $field['choices'] ) ) : ?>
								<?php foreach ( $field['choices'] as $k => $label ) : ?>
									<option value="<?php echo esc_attr( $k ); ?>" <?php selected( $k, $value ); ?>><?php echo esc_html( $label ); ?></option>
								<?php endforeach; ?>
						...
						<?php if ( isset( $field['description'] ) && ! empty( $field['description'] ) ) : ?>
							<span class="widget-field-description"><em><?php echo esc_html( $field['description'] ); ?></em></span>
						<?php endif; ?>
                         <span class="pro_text widget-field-description"><a href="<?php echo ATA_PRO;?>" target="_blank">upgrade pro to unlock</a></span>
					</p>
                   <?php endif;?>
					<?php
				break;
				...
				case 'dropdown-taxonomies':
					...
					?>
					<p class="widget-field-dropdown-taxonomies ata_wrp_select">
						<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $field['label'] ); ?></label>
					
                    <select id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" class="<?php echo esc_attr( $class ); ?>" disabled="disabled">
                       <option value="0"><?php echo esc_html( $tax_args['show_option_all'] ); ?></option>
                    </select>
                         <span class="pro_text widget-field-description"><a href="<?php echo ATA_PRO;?>" target="_blank">upgrade pro to unlock</a></span>
					</p>
					<?php
