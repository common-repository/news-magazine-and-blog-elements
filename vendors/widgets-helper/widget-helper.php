...
function magazine_point_widget_scripts( $hook ) {
	...
	if ( 'widgets.php' === $hook ) {
		...
		// Custom.
		wp_enqueue_style( 'newsgreen-widget-helper', get_template_directory_uri() . '/vendors/widget-helper/widget-helper' . $min . '.css', array(), '1.0.0' );
		wp_enqueue_script( 'newsgreen-widget-helper', get_template_directory_uri() . '/vendors/widget-helper/widget-helper' . $min . '.js', array( 'jquery' ), '1.0.0', true );
	}
