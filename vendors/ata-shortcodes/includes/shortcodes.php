...
if ( !function_exists('zilla_column') ) {
	function zilla_column( $atts, $content = null ) {
		...
		$last_div = '';
		if( $last ) {
			$last_class = ' zilla-column-last';
			$last_div = '<div class="clear"></div>';
		}

		return '<div class="zilla-' . $column . $last_class . '">' . do_shortcode($content) . '</div>' . $last_div;
	}
	add_shortcode('zilla_column', 'zilla_column');
}
