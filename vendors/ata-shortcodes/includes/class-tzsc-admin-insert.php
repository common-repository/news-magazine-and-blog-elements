...
/**
 * ATA_Admin_Insert class
 */
class ATA_Admin_Insert {
	...
	public function tzsc_build_fields($key, $param) {
		$default = ( isset( $param['default'] ) && $param['default'] != "" ) ? $param['default'] : '';
		$desc = ( isset( $param['desc'] ) && $param['desc'] != "" )? $param['desc']:'';
		...
		switch( $param['type'] )
		{
			case 'text' :
				...
				$output .= '<input type="text" class="zilla-form-text zilla-input" name="' . $key . '" id="' . $key . '" value="' .$default . '" />' . "\n";
				$output .= '<span class="zilla-form-desc">' . $desc . '</span></td>' . "\n";
			...
			case 'number' :
				...
				// prepare
				$output = '<td><label class="screen-reader-text" for="' . $key .'">' . $param['label'] . '</label>';
				$output .= '<input type="number" class="zilla-form-text zilla-input" name="' . $key . '" id="' . $key . '" value="' . $default . '" />' . "\n";
				$output .= '<span class="zilla-form-desc">' . $desc . '</span></td>' . "\n";
			...
			case 'select' :
				...
				$output = '<td><label class="screen-reader-text" for="' . $key .'">' . $param['label'] . '</label>';
				$output .= '<select name="' . $key . '" id="' . $key . '" class="zilla-form-select zilla-input" disabled="disabled">' . "\n";
				...
				$output .= '</select>' . "\n";
				$output .= '<span class="zilla-form-desc">' . $desc . '</span><span class="pro_text widget-field-description"><a href="'.ATA_PRO.'" target="_blank">upgrade pro to unlock</a></span></td>' . "\n";
			...
			case 'checkbox' :
				$default = ( isset( $param['default'] ) && $param['default'] != "" ) ? $param['default'] : 'false';
				// prepare
				$output = '<td><label class="screen-reader-text" for="' . $key .'">' . $param['label'] . '</label>';
				...
				$output .= '<option value="">'.esc_html__( 'False', 'newsgreen' ).'</option>' . "\n";
				
				$selected =  ( 'true' == $default ) ? 'selected="selected"' : '';
				$output .= '<option value="true"  '.$selected.'>'.esc_html__( 'True', 'newsgreen' ).'</option>' . "\n";
				...
				case 'dropdown-taxonomies':
					...
					$tax_args['order']        = ( isset( $param['order'] ) ) ? esc_attr( $param['order'] ) : 'ASC' ;
					$tax_args['show_option_all'] = ( isset( $param['show_option_all'] ) ) ? esc_html( $param['show_option_all'] ) : '' ;
					...
					$output .= '<option value="0">' . $tax_args['show_option_all'] . '</option>' . "\n";
					if( count ( $cats ) ){
						foreach ($cats as $row){
	...
	function tzsc_popup_html() {
		global $pagenow;
		include(ATA_PLUGIN_DIR . 'includes/config.php');
